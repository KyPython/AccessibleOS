...
import { Task, TaskCategory, TaskStats } from '../types';
...
export const useTasks = (options: UseTasksOptions = {}): UseTasksReturn => {
  ...
  const fetchTasks = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        const token = (await import('../contexts/AuthContext')).useAuth().getToken();
        const body = await apiClient.get('/tasks', token);
        if (body && body.data) {
          setTasks(body.data || []);
      ...
      const newStats: TaskStats = {
        ...
        completedThisWeek: allTasks.filter(task => task.status === 'completed').length,
        completedThisMonth: allTasks.filter(task => task.status === 'completed').length
      };
  ...
  const createTask = async (taskData: Partial<Task>): Promise<Task> => {
    ...
    try {
  const { getToken } = (await import('../contexts/AuthContext')).useAuth();
  const t = getToken();
  const body = await apiClient.post('/tasks', taskData, t);
  ...
  const { getToken } = (await import('../contexts/AuthContext')).useAuth();
  const t = getToken();
  const body = await apiClient.put(`/tasks/${taskId}`, updates, t);
  ...
  const { getToken } = (await import('../contexts/AuthContext')).useAuth();
  const t = getToken();
  await apiClient.del(`/tasks/${taskId}`, t);
